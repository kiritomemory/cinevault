...
import { Star, Clock, Calendar, Globe, DollarSign, Heart, BookmarkCheck, Eye, Plus, ChevronLeft, Play, Image as ImageIcon } from "lucide-react";
import { useParams, useLocation, useNavigate } from "react-router-dom";
import { getMovieDetails, getTvDetails, getImageUrl, formatDate, formatRuntime, formatCurrency, formatVote } from "@/services/tmdb";
import { saveMovie, saveTvShow, getMarking, setMarking, getLists, addToList } from "@/services/db";
import { useAppStore } from "@/stores/appStore";
import type { Movie, TVShow, CastMember, CrewMember, Video, ImageItem } from "@/types";
import { showToast } from "@/components/Toast";

type Tab = "overview" | "cast" | "media" | "recommendations";
...
export default function MovieDetailPage() {
  ...
  const saveRating = async () => {
    if (!data) return;
    await setMarking(numericId, isMovie ? "movie" : "tv", {
      user_rating: ratingValue || null,
      review_text: reviewText.trim() || null,
    });
    const updated = await getMarking(numericId, isMovie ? "movie" : "tv");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const detail = isMovie ? await getMovieDetails(numericId) : await getTvDetails(numericId);
        setData(detail);
        setSelected({ ...detail, media_type: isMovie ? "movie" : "tv" });
        if (isMovie) await saveMovie(detail);
        else await saveTvShow(detail);
        const m = await getMarking(numericId, isMovie ? "movie" : "tv");
        setMarkingState(m);
  ...
  const toggleMark = async (field: "is_watched" | "is_watchlist" | "is_favorite") => {
    ...
    await setMarking(numericId, isMovie ? "movie" : "tv", { [field]: !current });
    const updated = await getMarking(numericId, isMovie ? "movie" : "tv");
    setMarkingState(updated);
  };
  ...
  const handleAddToList = async (listId: number) => {
    if (!data) return;
    try {
      await addToList(listId, numericId, isMovie ? "movie" : "tv");
  ...
  const handleViewRecommendation = (r: any) => {
    navigate(`/${r.media_type || "movie"}/${r.id}`);
  };
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
        ...
        <button onClick={handleGoBack} className="text-primary text-sm hover:underline">返回首页</button>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground">
        <p>未找到相关内容</p>
        <button onClick={handleGoBack} className="mt-2 text-primary text-sm hover:underline">返回首页</button>
      </div>
    );
  }

  const title = isMovie ? (data as Movie).title : (data as TVShow).name;
  const originalTitle = isMovie ? (data as Movie).original_title : (data as TVShow).original_name;
  const date = isMovie ? (data as Movie).release_date : (data as TVShow).first_air_date;
  const year = (date || "").split("-")[0];
  const displayTitle = title + (year ? `.${year}` : "") + `.[tmdb=${data.id}]`;
  const runtime = isMovie ? (data as Movie).runtime : (data as TVShow).episode_run_time?.[0];
  const tagline = (data as any).tagline;
  const budget = (data as Movie).budget;
  const revenue = (data as Movie).revenue;
  const status = (data as any).status;
  const genres = (data as any).genres || [];
  const countries = (data as Movie).production_countries || [];
  ...
  const recommendations = ((data as any).recommendations?.results || []).map((r: any) => ({ ...r, media_type: r.media_type || "movie" }));
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="sticky top-0 z-10 glass-panel px-6">
        <div className="flex gap-6">
          {(["overview", "cast", "media", "recommendations"] as Tab[]).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === "overview" ? "概览" : t === "cast" ? "演职人员" : t === "media" ? "媒体" : "相关推荐"}
            </button>
          ))}
        </div>
      </div>
        ...
        {tab === "overview" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass-panel p-4 rounded-xl">
              ...
              {showRatingForm && (
                <div className="mt-4 p-3 rounded-lg bg-accent/50">
                  ...
                  <div className="flex gap-2">
                    <button onClick={saveRating} className="px-3 py-1 rounded bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 transition-opacity">
                      保存
                    </button>
                    <button onClick={() => setShowRatingForm(false)} className="px-3 py-1 rounded border border-border text-xs hover:bg-accent transition-colors">
                      取消
                    </button>
                  </div>
                </div>
              ...
              {!marking?.user_rating && !showRatingForm && (
                <button onClick={openRatingForm} className="mt-3 text-xs text-primary hover:underline">
                  ✦ 写评分
                </button>
              )}
            </div>
            <div className="space-y-3 text-sm">
              <div className="flex items-center gap-2 text-muted-foreground">
                <Calendar size={14} /> <span className="text-foreground font-medium">首映日期:</span> {formatDate(date)}
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <Clock size={14} /> <span className="text-foreground font-medium">片长:</span> {formatRuntime(runtime)}
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
              ...
              {isMovie && (
                <>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <DollarSign size={14} /> <span className="text-foreground font-medium">预算:</span> {formatCurrency(budget)}
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <DollarSign size={14} /> <span className="text-foreground font-medium">票房:</span> {formatCurrency(revenue)}
                  </div>
                </>
              )}
              {status && (
              ...
              {countries.length > 0 && (
                <div className="flex items-center gap-2 text-muted-foreground">
                  <span className="text-foreground font-medium">制作国家:</span> {countries.map((c: any) => c.name).join(", ")}
                </div>
              )}
            </div>
          </div>
